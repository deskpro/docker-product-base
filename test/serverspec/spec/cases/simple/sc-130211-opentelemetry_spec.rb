...
describe "Case SC-130211: Enable OTEL" do
  ...
  after(:all) do
    FileUtils.remove_file('/etc/php/8.3/cli/conf.d/zzz-test.ini', true)
  ...
  it "DESKPRO_ENABLE_OTEL should activate opentelemetry extension" do
    ...
    output = `eval-tpl -f /etc/php/8.3/mods-available/deskpro-otel.ini.tmpl`
    expect(output).to contain "opentelemetry extension is enabled"

    `eval-tpl -f /etc/php/8.3/mods-available/deskpro-otel.ini.tmpl > /etc/php/8.3/cli/conf.d/zzz-test.ini`
